---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for QingClaws deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, QingClaws cron scheduling for periodic checks, or version status checks on a machine running QingClaws (laptop, workstation, Pi, VPS).
---

# QingClaws Host Hardening

## Overview

Assess and harden the host running QingClaws, then align it to a user-defined risk tolerance without breaking access. Use QingClaws security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim QingClaws changes the host firewall, SSH, or OS updates; it does not.
...
5. QingClaws gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if QingClaws can access sensitive data.
...
### 2) Run QingClaws security audits (read-only)

As part of the default read-only checks, run `qingclaws security audit --deep`. Only offer alternatives if the user requests them:

1. `qingclaws security audit` (faster, non-probing)
2. `qingclaws security audit --json` (structured output)

Offer to apply QingClaws safe defaults (numbered):

1. `qingclaws security audit --fix`

Be explicit that `--fix` only tightens QingClaws defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check QingClaws version/update status (read-only)

As part of the default read-only checks, run `qingclaws update status`.
...
- Credential hygiene notes (location of QingClaws creds, prefer disk encryption)
...
- QingClaws security audit (re-run)
...
After QingClaws install or first hardening pass, run at least one baseline audit and version check:

- `qingclaws security audit`
- `qingclaws security audit --deep`
- `qingclaws update status`

Ongoing monitoring is recommended. Use the QingClaws cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `qingclaws cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `qingclaws update status`
...
Before creating, `qingclaws cron list` and match on exact `name`. If found, `qingclaws cron edit <id> ...`.
If not found, `qingclaws cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `qingclaws update status` (preferred for source checkouts and channels)
2. `npm view qingclaws version` (published npm version)

## QingClaws command accuracy

Use only supported commands and flags:

- `qingclaws security audit [--deep] [--fix] [--json]`
- `qingclaws status` / `qingclaws status --deep`
- `qingclaws health --json`
- `qingclaws update status`
- `qingclaws cron add|list|runs|run`

Do not invent CLI flags or imply QingClaws enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by QingClaws compaction:
