---
summary: "CLI reference for `qingclaws onboard` (interactive onboarding wizard)"
read_when:
...
```bash
qingclaws onboard
qingclaws onboard --flow quickstart
qingclaws onboard --flow manual
qingclaws onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `qingclaws dashboard` (Control UI, no channel setup).
