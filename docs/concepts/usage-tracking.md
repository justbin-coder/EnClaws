...
- `/usage cost` in chats: local cost summary aggregated from QingClaws session logs.
- CLI: `qingclaws status --usage` prints a full per-provider breakdown.
- CLI: `qingclaws channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
