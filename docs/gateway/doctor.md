...
`qingclaws doctor` is the repair + migration tool for QingClaws. It fixes stale
...
```bash
qingclaws doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
qingclaws doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
qingclaws doctor --repair --force
```
...
- Extra workspace dir detection (`~/qingclaws`).
...
- Sessions store + transcripts:
  - from `~/.qingclaws/sessions/` to `~/.qingclaws/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.qingclaws/agent/` to `~/.qingclaws/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.qingclaws/credentials/*.json` (except `oauth.json`)
  - to `~/.qingclaws/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `qingclaws doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.qingclaws` folders exist across
  home directories or when `QINGCLAWS_STATE_DIR` points elsewhere (history can
...
offers to remove them and install the QingClaws service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named QingClaws gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `qingclaws doctor --generate-gateway-token` to force token
...
- `qingclaws doctor` prompts before rewriting supervisor config.
- `qingclaws doctor --yes` accepts the default repair prompts.
- `qingclaws doctor --repair` applies recommended fixes without prompts.
- `qingclaws doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `qingclaws gateway install --force`.
