...
- `qingclaws gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `qingclaws doctor` reports no blocking config/service issues.
- `qingclaws channels status --probe` shows connected/ready channels.
...
qingclaws config get agents.defaults.models
```
...
qingclaws system heartbeat last
qingclaws logs --follow
```
...
qingclaws approvals get --node <idOrNameOrIp>
qingclaws logs --follow
qingclaws status
```
...
qingclaws browser start --browser-profile qingclaws
...
```bash
qingclaws gateway status
qingclaws config get gateway.mode
qingclaws config get gateway.remote.url
qingclaws config get gateway.auth.mode
```
...
```bash
qingclaws config get gateway.bind
qingclaws config get gateway.auth.token
qingclaws gateway status
qingclaws logs --follow
```
