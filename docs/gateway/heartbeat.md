...
- During heartbeat runs, QingClaws treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `qingclaws sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, QingClaws skips the heartbeat run entirely (no model call).
...
```bash
qingclaws system event --text "Check for urgent follow-ups" --mode now
```
