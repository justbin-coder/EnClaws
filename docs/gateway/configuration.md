...
If the file is missing, QingClaws uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `qingclaws onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    qingclaws onboard       # full setup wizard
    qingclaws configure     # config wizard
    ```
  </Tab>
...
<Warning>
QingClaws only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`qingclaws doctor`, `qingclaws logs`, `qingclaws health`, `qingclaws status`)
- Run `qingclaws doctor` to see exact issues
- Run `qingclaws doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `qingclaws config set` for single keys.
    </Warning>
    ...
    qingclaws gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.qingclaws/workspace\" } } }",
      "baseHash": "<hash>",
...
QingClaws reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.qingclaws/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, QingClaws runs your login shell and imports only the missing keys:
...
Env var equivalent: `QINGCLAWS_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${QINGCLAWS_GATEWAY_TOKEN}" } },
