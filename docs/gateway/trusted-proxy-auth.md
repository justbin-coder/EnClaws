...
read_when:
  - Running QingClaws behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of QingClaws
...
- You run QingClaws behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. QingClaws checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. QingClaws extracts the user identity from the configured header
...
- QingClaws can stay on loopback HTTP behind the proxy.
...
If QingClaws itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://qingclaws.example.com
    to: http://qingclaws-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
qingclaws.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy qingclaws:18789 {
...
location / {
    ...
    proxy_pass http://qingclaws:18789;
...
`qingclaws security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update QingClaws config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `qingclaws security audit` and review findings
