...
read_when:
  - Designing workspace memory (~/.qingclaws/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep QingClaws integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.qingclaws/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.qingclaws/workspace` as canonical human-readable memory.
...
```
~/.qingclaws/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in QingClaws**, but keep a separable core library.

### Why integrate into QingClaws?

- QingClaws already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `qingclaws memory recall "…" --k 25 --since 30d`
  - `qingclaws memory reflect --since 7d`
...
Pragmatic take for `~/.qingclaws/workspace`:
