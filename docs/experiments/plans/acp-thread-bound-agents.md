...
This plan defines how QingClaws should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- QingClaws core owns ACP control plane concerns
...
QingClaws should not reimplement ACP transport internals in core.
QingClaws should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in QingClaws, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under QingClaws state dir
...
Core (must be in QingClaws core):
...
- ACP event stream is normalized to QingClaws reply chunks
...
For the first production backend (`extensions/acpx`), QingClaws and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses QingClaws `sessionKey`)
...
- QingClaws consumes ndjson events from `acpx --format json --json-strict`
