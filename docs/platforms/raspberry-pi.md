---
summary: "QingClaws on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up QingClaws on a Raspberry Pi
  - Running QingClaws on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# QingClaws on Raspberry Pi

## Goal

Run a persistent, always-on QingClaws Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
qingclaws config set gateway.bind tailnet
sudo systemctl restart qingclaws
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/qingclaws-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `QINGCLAWS_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running QingClaws, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit qingclaws
```

```ini
[Service]
Environment=QINGCLAWS_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/qingclaws-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep QingClaws state/cache on SSD-backed storage to avoid SD-card
...
Most QingClaws features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled qingclaws
...
```bash
# Check logs
journalctl -u qingclaws --no-pager -n 100

# Common fix: rebuild
cd ~/qingclaws  # if using hackable install
npm run build
sudo systemctl restart qingclaws
```
