---
summary: "QingClaws on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up QingClaws on DigitalOcean
  - Looking for cheap VPS hosting for QingClaws
title: "DigitalOcean"
---

# QingClaws on DigitalOcean

## Goal

Run a persistent QingClaws Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status qingclaws-gateway.service

# View logs
journalctl --user -u qingclaws-gateway.service -f
```
...
```bash
qingclaws channels login whatsapp
# Scan QR code
```
...
- `~/.qingclaws/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u qingclaws --no-pager -n 50
```
