...
- Same path: run the app from a fixed location (for QingClaws, `dist/QingClaws.app`).
...
```bash
sudo tccutil reset Accessibility ai.qingclaws.mac
sudo tccutil reset ScreenCapture ai.qingclaws.mac
...
Workaround: move files into the QingClaws workspace (`~/.qingclaws/workspace`) if you want to avoid per-folder grants.
