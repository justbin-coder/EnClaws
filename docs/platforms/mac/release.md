---
summary: "QingClaws macOS release checklist (Sparkle feed, packaging, signing)"
read_when:
  - Cutting or validating a QingClaws macOS release
  - Updating the Sparkle appcast or feed assets
title: "macOS Release"
---

# QingClaws macOS release (Sparkle)
...
- Notary credentials (keychain profile or API key) for `xcrun notarytool` if you want Gatekeeper-safe DMG/zip distribution.
  - We use a Keychain profile named `qingclaws-notary`, created from App Store Connect API key env vars in your shell profile:
    - `APP_STORE_CONNECT_API_KEY_P8`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`
    - `echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/qingclaws-notary.p8`
    - `xcrun notarytool store-credentials "qingclaws-notary" --key /tmp/qingclaws-notary.p8 --key-id "$APP_STORE_CONNECT_KEY_ID" --issuer "$APP_STORE_CONNECT_ISSUER_ID"`
...
ditto -c -k --sequesterRsrc --keepParent dist/QingClaws.app dist/QingClaws-2026.3.1.zip

# Optional: also build a styled DMG for humans (drag to /Applications)
scripts/create-dmg.sh dist/QingClaws.app dist/QingClaws-2026.3.1.dmg

# Recommended: build + notarize/staple zip + DMG
# First, create a keychain profile once:
#   xcrun notarytool store-credentials "qingclaws-notary" \
#     --apple-id "<apple-id>" --team-id "<team-id>" --password "<app-specific-password>"
NOTARIZE=1 NOTARYTOOL_PROFILE=qingclaws-notary \
BUNDLE_ID=ai.qingclaws.mac \
APP_VERSION=2026.3.1 \
BUILD_CONFIG=release \
SIGN_IDENTITY="Developer ID Application: <Developer Name> (<TEAMID>)" \
...
ditto -c -k --keepParent apps/macos/.build/release/QingClaws.app.dSYM dist/QingClaws-2026.3.1.dSYM.zip
```
...
- Upload `QingClaws-2026.3.1.zip` (and `QingClaws-2026.3.1.dSYM.zip`) to the GitHub release for tag `v2026.3.1`.
