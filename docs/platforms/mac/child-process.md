...
service via the external `qingclaws` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.qingclaws.gateway`
  (or `ai.qingclaws.<profile>` when using `--profile`/`QINGCLAWS_PROFILE`; legacy `com.qingclaws.*` is supported).
