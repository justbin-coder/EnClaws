...
summary: "Architecture of QingClaws's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in QingClaws
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how QingClaws integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

QingClaws uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, QingClaws directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in QingClaws's local TUI mode)                                             |
...
├── pi-tools.ts                    # createQingClawsCodingTools()
...
├── qingclaws-tools.ts              # QingClaws-specific tools
...
Image injection is prompt-local: QingClaws loads image refs from the current prompt and
...
2. **Custom Replacements**: QingClaws replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **QingClaws Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures QingClaws's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, QingClaws CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
QingClaws wraps this with `guardSessionManager()` for tool result safety.
...
QingClaws maintains an auth profile store with multiple API keys per provider:
...
QingClaws loads custom pi extensions for specialized behavior:
...
QingClaws also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | QingClaws Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom QingClaws tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.qingclaws/agents/<agentId>/sessions/` (or `$QINGCLAWS_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `QINGCLAWS_LIVE_TEST=1`)
