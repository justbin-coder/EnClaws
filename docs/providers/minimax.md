---
summary: "Use MiniMax M2.1 in QingClaws"
read_when:
  - You want MiniMax models in QingClaws
...
```bash
qingclaws plugins enable minimax-portal-auth  # skip if already loaded.
qingclaws gateway restart  # restart if gateway is already running
qingclaws onboard --auth-choice minimax-portal
```
...
Configure manually via `qingclaws.json`:
...
- Use `qingclaws models list` and `qingclaws models set minimax/MiniMax-M2.1` to switch.
...
- Running `qingclaws configure` and selecting **MiniMax M2.1**, or
