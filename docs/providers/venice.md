---
summary: "Use Venice AI privacy-focused models in QingClaws"
read_when:
  - You want privacy-focused inference in QingClaws
...
```bash
qingclaws onboard --auth-choice venice-api-key
```
...
```bash
qingclaws onboard --non-interactive \
...
```bash
qingclaws agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, QingClaws shows all available Venice models. Pick based on your needs:
...
```bash
qingclaws models set venice/claude-opus-45
qingclaws models set venice/llama-3.3-70b
```

List all available models:

```bash
qingclaws models list | grep venice
```

## Configure via `qingclaws configure`

1. Run `qingclaws configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
QingClaws automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
qingclaws agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
qingclaws agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
qingclaws agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
qingclaws agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
qingclaws agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `qingclaws models list` to see currently available models. Some models may be temporarily offline.
