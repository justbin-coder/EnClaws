...
read_when:
  - You want to place an outbound voice call from QingClaws
...
Voice calls for QingClaws via a plugin. Supports outbound notifications and
...
- Use `qingclaws voicecall ...` or the `voice_call` tool
...
```bash
qingclaws plugins install ./extensions/voice-call
