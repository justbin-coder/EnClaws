...
- **gateway host** → `qingclaws` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.qingclaws/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.qingclaws/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `qingclaws approvals allowlist ...`).
- `qingclaws security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `qingclaws doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.qingclaws/exec-approvals.json` directly.

CLI: `qingclaws approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
