...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `qingclaws sandbox explain`.
...
Legacy `agent.*` configs are migrated by `qingclaws doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=qingclaws-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${QINGCLAWS_STATE_DIR:-$HOME/.qingclaws}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
