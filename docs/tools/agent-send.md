---
summary: "Direct `qingclaws agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `qingclaws agent` (direct agent runs)

`qingclaws agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `qingclaws message --target`).
...
```bash
qingclaws agent --to +15555550123 --message "status update"
qingclaws agent --agent ops --message "Summarize logs"
...
qingclaws agent --to +15555550123 --message "Summon reply" --deliver
