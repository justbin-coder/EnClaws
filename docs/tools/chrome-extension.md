---
summary: "Chrome extension: let QingClaws drive your existing Chrome tab"
read_when:
...
The QingClaws Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate qingclaws-managed Chrome profile.
...
QingClaws then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the QingClaws release (npm package) as static files. There is no separate “build” step.

After upgrading QingClaws:

- Re-run `qingclaws browser extension install` to refresh the installed files under your QingClaws state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

QingClaws ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `QINGCLAWS_GATEWAY_TOKEN`)
...
- Open the tab you want QingClaws to control.
...
- `ON`: attached; QingClaws can drive that tab.
...
Debugging: `qingclaws sandbox explain`
...
`qingclaws browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `qingclaws browser extension install` first to copy the extension to a stable location under your QingClaws state directory.
...
- **This is not isolated** like the dedicated qingclaws-managed profile.
