...
- `gateway`/`node` approvals are controlled by `~/.qingclaws/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  QingClaws prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`qingclaws security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `qingclaws doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
