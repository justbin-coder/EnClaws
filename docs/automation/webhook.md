...
read_when:
  ...
  - Wiring external systems into QingClaws
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your QingClaws config directory (typically `~/.qingclaws/hooks/transforms`).
...
- `qingclaws webhooks gmail setup` writes `hooks.gmail` config for `qingclaws webhooks gmail run`.
