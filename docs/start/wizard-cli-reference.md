...
read_when:
  - You need detailed behavior for qingclaws onboard
...
This page is the full reference for `qingclaws onboard`.
...
<Steps>
  <Step title="Existing config detection">
    ...
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `qingclaws doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `qingclaws pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `qingclaws health`.
    - `qingclaws status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.qingclaws/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.qingclaws/agents/<agentId>/agent/auth-profiles.json`
...
`~/.qingclaws/credentials/oauth.json` (or `$QINGCLAWS_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`qingclaws onboard`](/cli/onboard)
