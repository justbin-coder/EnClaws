...
QingClaws 是基于 EnClaws v0.2.0（hashSTACS-Global 对 OpenClaw 的二次品牌化产物）的 fork，目标是公司独立品牌的 2B 企业级 Agent 平台底座。spec §11 把品牌化前置为 PE1，理由：先做品牌化，后续 PE2-PE7 的产出（CLI 提示 / log / 文档 / 默认模板）天然带 QingClaws 品牌，避免回头改。

仓库现状扫描：
- `openclaw / OpenClaw / OPENCLAW` 在 src/ui/docs/extensions/skills/scripts 共 ~16,335 hits
- `enclaws / EnClaws` 在 src/ui/docs/scripts 共 ~4,555 hits（extensions/skills 未被 hashSTACS 覆盖）
- `hashSTACS-Global / hashSTACS` 出现在 9 文件（README / NOTICE / scripts / package.json）
...
| D4 | 待替换字符串 | `openclaw/OpenClaw/OPENCLAW` 全替换；`enclaws/EnClaws` 替换（仅"上游归属语句"段保留）；`hashSTACS-Global / hashSTACS` UI/banner/CLI 替换（LICENSE/NOTICE 保留并追加 QingClaws 归属）；`openclaw.ai` 域名变量化 + 默认占位空 |
| D5 | 迁移兼容 | **不保留 openclaw 别名、不写迁移工具**；本机 `~/.openclaw/` / `~/.enclaws/` 直接重建为 `~/.qingclaws/`（fork 早期 + 客户尚未部署） |
...
| 用户可见层（全量替换为 QingClaws） | 源码内部层（保留 openclaw 标识 + 注释标记） |
|---|---|
| `package.json` name / bin / scripts | 内部 `import` 路径中的 `"openclaw"` npm 包内部模块名 |
| CLI 可执行名（`enclaws.mjs` → `qingclaws.mjs`） | 类名 / 函数名 / 内部变量 |
| 配置目录（`~/.openclaw/` / `~/.enclaws/`）→ `~/.qingclaws/` | 测试 `describe/it` 块的旧名（避免 git mv 历史断裂） |
...
- 这两件事是日后合并 EnClaws / OpenClaw 上游的"导航图"
...
| 位置 | 注入方式 | 例子 |
|---|---|---|
| 编译期固定字符串（`package.json` / 构建配置 / Docker tag） | codemod 物理写入 | `package.json` `name` 字段；`enclaws.mjs` → `qingclaws.mjs` |
...
3. 对用户面层做正则替换：
   - openclaw / OpenClaw / OPENCLAW → qingclaws / QingClaws / QINGCLAWS
   - enclaws / EnClaws → qingclaws / QingClaws
   - hashSTACS-Global / hashSTACS → QingClaws Team（仅 UI/banner/CLI；NOTICE/LICENSE 保留并追加）
...
```bash
# 在用户可见层文件 rg 是否还有 openclaw / enclaws / hashSTACS 字样
...
| # | Commit | 涉及范围 | 工作量 |
|---|---|---|---:|
| 1 | `chore(docs): slim CLAUDE.md and remove redundant memory` | MD 瘦身（`EnClaws/CLAUDE.md` 删与 AGENTS.md 重复章节 ~80 行；`CLAUDE.local.md` §2 放宽 + §3 加冲突消解；删冗余 memory + 更新 MEMORY.md 索引） | 0.5d |
...
| 3 | `feat(PE1): rebrand user-facing surface (CLI / pkg / paths / env)` | 运行 codemod 做机械替换 + 手工核校：`package.json` name/bin/scripts、`bin: enclaws.mjs → qingclaws.mjs`（git mv）、CLI 欢迎/启动 banner/错误信息、环境变量、UI 标题栏/About/启动页、Docker/launchd/GHA 用户面 label、HTTP API 路径 | 2-3d |
...
**NOTICE 预期格式**：保留全部上游 attribution 段不删一字，**追加** QingClaws attribution 段（说明 QingClaws 基于 EnClaws fork、Apache-2.0 协议、版权所有者）。
...
| `tests/branding/cli-smoke.test.ts` | `qingclaws --version` / `--help` / `gateway --help` 三条命令 | 输出含 `QingClaws`，不含 `openclaw/enclaws/hashSTACS` |
| `tests/branding/startup-banner.snapshot.ts` | 启动 gateway 抓 stdout banner、UI About、UI 启动页标题 | 对照 snapshot 一致；含 QingClaws |
| `tests/branding/license-attribution.test.ts` | 解析 `LICENSE` / `NOTICE` 头部 | 含 QingClaws 归属 **且** 含 EnClaws / hashSTACS-Global / OpenClaw 上游归属 |
...
- 手工启动 gateway + UI demo（截图存档），确认：
  ...
  - About 弹窗无 EnClaws / hashSTACS / OpenClaw / 客户特定词汇
  - `~/.qingclaws/` 目录创建成功
...
- ❌ 上游 EnClaws v0.3+ 版本 merge（属上游跟进策略，非 PE1）
