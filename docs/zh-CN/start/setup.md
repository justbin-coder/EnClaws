...
summary: 设置指南：在保持最新的同时保持你的 QingClaws 设置个性化
title: 设置
x-i18n:
...
- **工作区：** `~/.qingclaws/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm qingclaws setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **QingClaws.app**（菜单栏）。
...
- 运行 `qingclaws setup`，然后 `qingclaws channels login`，然后手动启动 Gateway 网关（`qingclaws gateway`）。
...
- **配对允许列表**：`~/.qingclaws/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.qingclaws/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.qingclaws/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.qingclaws/workspace` 和 `~/.qingclaws/` 保持为"你的东西"；不要将个人提示/配置放入 `qingclaws` 仓库。
...
- [QingClaws 助手设置](/start/qingclaws)
