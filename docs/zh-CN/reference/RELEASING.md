...
- [ ] `QINGCLAWS_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`（Docker 安装冒烟测试，快速路径；发布前必需）
  - 如果已知上一个 npm 发布版本有问题，为预安装步骤设置 `QINGCLAWS_INSTALL_SMOKE_PREVIOUS=<last-good-version>` 或 `QINGCLAWS_INSTALL_SMOKE_SKIP_PREVIOUS=1`。
- [ ]（可选）完整安装程序冒烟测试（添加非 root + CLI 覆盖）：`pnpm test:install:smoke`
- [ ]（可选）安装程序 E2E（Docker，运行 `curl -fsSL https://qingclaws.ai/install.sh | bash`，新手引导，然后运行真实工具调用）：
...
- [ ] 按照 [macOS 发布](/platforms/mac/release) 获取确切命令和所需环境变量。
  ...
  - 如果进行公证，使用从 App Store Connect API 环境变量创建的 `qingclaws-notary` 钥匙串配置文件（参见 [macOS 发布](/platforms/mac/release)）。
...
- [ ] 验证注册表：`npm view qingclaws version`、`npm view qingclaws dist-tags` 和 `npx -y qingclaws@X.Y.Z --version`（或 `--help`）。

### 故障排除（来自 2.0.0-beta2 发布的笔记）

- **npm pack/publish 挂起或产生巨大 tarball**：`dist/QingClaws.app` 中的 macOS 应用包（和发布 zip）被扫入包中。通过 `package.json` 的 `files` 白名单发布内容来修复（包含 dist 子目录、docs、skills；排除应用包）。用 `npm pack --dry-run` 确认 `dist/QingClaws.app` 未列出。
...
- [ ] 为 `vX.Y.Z` 创建/刷新 GitHub 发布，**标题为 `qingclaws X.Y.Z`**（不仅仅是标签）；正文应包含该版本的**完整**变更日志部分（亮点 + 更改 + 修复），内联显示（无裸链接），且**不得在正文中重复标题**。
- [ ] 附加产物：`npm pack` tarball（可选）、`QingClaws-X.Y.Z.zip` 和 `QingClaws-X.Y.Z.dSYM.zip`（如果生成）。
- [ ] 提交更新后的 `appcast.xml` 并推送（Sparkle 从 main 获取源）。
- [ ] 从干净的临时目录（无 `package.json`），运行 `npx -y qingclaws@X.Y.Z send --help` 确认安装/CLI 入口点正常工作。
- [ ] 宣布/分享发布说明。

## 插件发布范围（npm）

我们只发布 `@qingclaws/*` 范围下的**现有 npm 插件**。不在 npm 上的内置插件保持**仅磁盘树**（仍在 `extensions/**` 中发布）。

获取列表的流程：

1. `npm search @qingclaws --json` 并捕获包名。
