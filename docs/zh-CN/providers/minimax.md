---
read_when:
  - 你想在 QingClaws 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 QingClaws 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
qingclaws plugins enable minimax-portal-auth  # 如果已加载则跳过
qingclaws gateway restart  # 如果 Gateway 网关已在运行则重启
...
- 使用 `qingclaws models list` 和 `qingclaws models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `qingclaws configure` 并选择 **MiniMax M2.1**，或
