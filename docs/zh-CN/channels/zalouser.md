...
- 通过 CLI 安装：`qingclaws plugins install @qingclaws/zalouser`
- 或从源码检出安装：`qingclaws plugins install ./extensions/zalouser`
- 详情：[插件](/tools/plugin)
...
- 配置向导可以提示输入群组允许列表。
- 启动时，QingClaws 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`qingclaws channels logout --channel zalouser && qingclaws channels login --channel zalouser`
