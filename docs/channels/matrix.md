...
Matrix is an open, decentralized messaging protocol. QingClaws connects as a Matrix **user**
...
```bash
qingclaws plugins install @qingclaws/matrix
```

Local checkout (when running from a git repo):

```bash
qingclaws plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `qingclaws plugins install @qingclaws/matrix`
   - From a local checkout: `qingclaws plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: QingClaws calls the same
     login endpoint, stores the access token in `~/.qingclaws/credentials/matrix/credentials.json`,
...
- On first connection, QingClaws requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  QingClaws logs a warning.
...
`~/.qingclaws/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, QingClaws resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
