...
```bash
qingclaws plugins install @qingclaws/msteams
```

Local checkout (when running from a git repo):

```bash
qingclaws plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
QingClaws will offer the local install path automatically.
...
3. Configure QingClaws with those credentials.
...
- Talk to QingClaws via Teams DMs, group chats, or channels.
...
- On startup, QingClaws resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring QingClaws, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `qingclaws-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `qingclaws plugins install @qingclaws/msteams`
   - From a local checkout: `qingclaws plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "QingClaws in Teams", "full": "QingClaws in Teams" },
...
QingClaws handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, QingClaws only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/QingClawsShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

QingClaws sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `qingclaws message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.qingclaws/msteams-polls.json`.
...
```bash
qingclaws message send --channel msteams \
...
qingclaws message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
qingclaws message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
qingclaws message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
qingclaws message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
