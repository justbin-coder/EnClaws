...
Tlon is a decentralized messenger built on Urbit. QingClaws connects to your Urbit ship and can
...
```bash
qingclaws plugins install @qingclaws/tlon
```

Local checkout (when running from a git repo):

```bash
qingclaws plugins install ./extensions/tlon
```
...
By default, QingClaws blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `qingclaws message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, QingClaws replies in-thread.
