...
read_when:
  ...
  - Reviewing QingClaws security posture
title: "Pairing"
---

# Pairing

“Pairing” is QingClaws’s explicit **owner approval** step.
...
Stored under `~/.qingclaws/credentials/`:
...
3. On your phone, open the QingClaws iOS app → Settings → Gateway.
...
Stored under `~/.qingclaws/devices/`:
...
- The legacy `node.pair.*` API (CLI: `qingclaws nodes pending/approve`) is a
