...
read_when:
  - Setting up Twitch chat integration for QingClaws
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. QingClaws connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
qingclaws plugins install @qingclaws/twitch
```

Local checkout (when running from a git repo):

```bash
qingclaws plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `QINGCLAWS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "qingclaws", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use QINGCLAWS_TWITCH_ACCESS_TOKEN env var)
...
```bash
QINGCLAWS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
