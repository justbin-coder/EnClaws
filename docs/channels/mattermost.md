---
summary: "Mattermost bot setup and QingClaws config"
read_when:
...
```bash
qingclaws plugins install @qingclaws/mattermost
```

Local checkout (when running from a git repo):

```bash
qingclaws plugins install ./extensions/mattermost
```
...
4. Configure QingClaws and start the gateway.
...
- Approve via:
  - `qingclaws pairing list mattermost`
  - `qingclaws pairing approve mattermost <CODE>`
...
Use these target formats with `qingclaws message send` or cron/webhooks:
