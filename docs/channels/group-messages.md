    ...
    list: [
      {
        ...
        groupChat: {
          historyLimit: 50,
          mentionPatterns: ["@?qingclaws", "\\+?15555550123"],
        },
      },
    ],
  },
...
- The regexes are case-insensitive; they cover a display-name ping like `@qingclaws` and the raw number with or without `+`/spaces.
...
1. Add your WhatsApp account (the one running QingClaws) to the group.
2. Say `@qingclaws …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@qingclaws` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.qingclaws/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
