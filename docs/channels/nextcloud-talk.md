...
```bash
qingclaws plugins install @qingclaws/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
qingclaws plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "QingClaws" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `qingclaws pairing list nextcloud-talk`
  - `qingclaws pairing approve nextcloud-talk <CODE>`
