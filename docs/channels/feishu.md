...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects QingClaws to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
qingclaws plugins install @qingclaws/feishu
```

Local checkout (when running from a git repo):

```bash
qingclaws plugins install ./extensions/feishu
```
...
If you just installed QingClaws, run the wizard:
...
2. Configuring app credentials in QingClaws
...
1. You already ran `qingclaws channels add` for Feishu
2. The gateway is running (`qingclaws gateway status`)
...
```bash
qingclaws pairing approve feishu <CODE>
```
...
2. Run `qingclaws logs --follow` and look for `chat_id`
...
2. Run `qingclaws logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `qingclaws gateway status`  | Show gateway status           |
| `qingclaws gateway install` | Install/start gateway service |
| `qingclaws gateway stop`    | Stop gateway service          |
| `qingclaws gateway restart` | Restart gateway service       |
| `qingclaws logs --follow`   | Tail gateway logs             |
...
4. Check logs: `qingclaws logs --follow`
...
5. Ensure the gateway is running: `qingclaws gateway status`
6. Check logs: `qingclaws logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.qingclaws/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.qingclaws/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
