...
```bash
qingclaws tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `qingclaws tui --deliver`
...
- Check the Gateway logs: `qingclaws logs --follow`.
- Confirm the agent can run: `qingclaws status` and `qingclaws models status`.
...
- No agents in picker: check `qingclaws agents list` and your routing config.
