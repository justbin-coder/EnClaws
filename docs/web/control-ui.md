...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/qingclaws`)
...
If the page fails to load, start the Gateway first: `qingclaws gateway`.
...
you revoke it with `qingclaws devices revoke --device <id> --role <role>`. See
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop qingclaws`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. QingClaws
...
QingClaws **blocks** Control UI connections without device identity.
