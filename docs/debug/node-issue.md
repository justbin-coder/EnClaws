...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in QingClaws
...
Running QingClaws via Node with `tsx` fails at startup with:

```
[qingclaws] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `qingclaws status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch qingclaws.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node qingclaws.mjs status` works on Node 25.
