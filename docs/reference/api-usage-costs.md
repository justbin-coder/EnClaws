...
QingClaws features that can generate provider usage or paid API calls.
...
- `qingclaws status --usage` and `qingclaws channels list` show provider **usage windows**
...
QingClaws can pick up credentials from:
...
- `qingclaws status --usage`
- `qingclaws models status --json`
...
`qingclaws models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
