...
QingClaws integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- QingClaws owns lifecycle when `channels.signal.autoStart=true`.
...
- QingClaws spawns `imsg rpc` as a child process (legacy iMessage integration).
