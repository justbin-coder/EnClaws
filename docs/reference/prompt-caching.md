...
- With Anthropic API-key auth profiles, QingClaws seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, QingClaws injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
QingClaws exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.qingclaws/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$QINGCLAWS_STATE_DIR/logs/cache-trace.jsonl`
...
- `QINGCLAWS_CACHE_TRACE=1` enables cache tracing.
- `QINGCLAWS_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `QINGCLAWS_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `QINGCLAWS_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `QINGCLAWS_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
