...
This document explains how QingClaws manages sessions end-to-end:
...
QingClaws is designed around a single **Gateway process** that owns session state.
...
QingClaws persists sessions in two layers:
...
- Transcripts: `~/.qingclaws/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

QingClaws resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, QingClaws reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.qingclaws/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
QingClaws intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (QingClaws consumes the events, but Pi decides when to compact).
...
QingClaws also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, QingClaws bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, QingClaws leaves it alone.
...
- `qingclaws status` (CLI)
- `qingclaws sessions` / `sessions --json`
...
QingClaws supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- QingClaws strips/suppresses this in the delivery layer.

As of `2026.1.10`, QingClaws also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
QingClaws uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but QingClaws’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `qingclaws status`.
