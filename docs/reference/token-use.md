---
summary: "How QingClaws builds prompt context and reports token usage + costs"
read_when:
...
QingClaws tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

QingClaws assembles its own system prompt on every run. It includes:
...
For images, QingClaws downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from QingClaws session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `qingclaws status --usage` and `qingclaws channels list` show
...
`cacheWrite`. If pricing is missing, QingClaws shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. QingClaws can
...
Anthropic's 1M context window is currently beta-gated. QingClaws can inject the
...
QingClaws skips the `context-1m-*` beta header because Anthropic currently
