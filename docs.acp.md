# QingClaws ACP Bridge

This document describes how the QingClaws ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`qingclaws acp` exposes an ACP agent over stdio and forwards prompts to a running
QingClaws Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `qingclaws acp` and speaks ACP messages over stdio.
...
`qingclaws acp` resolves the Gateway URL and auth from CLI flags or config:
