...
function resolveGatewayPort(cfg: QingClawsPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.QINGCLAWS_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: QingClawsPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: QingClawsPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the QingClaws iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the QingClaws iOS app:",
            "",
            "```",
            qrAscii,
            "```",
