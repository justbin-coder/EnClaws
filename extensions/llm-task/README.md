...
`qingclaws.invoke --each`) without adding new QingClaws code per workflow.
...
This extension depends on QingClaws internal modules (the embedded agent runner).
It is intended to ship as a **bundled** QingClaws extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.qingclaws/extensions` as a standalone plugin directory.
