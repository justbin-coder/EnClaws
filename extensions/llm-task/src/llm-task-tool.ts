...
import Ajv from "ajv";
import { resolvePreferredQingClawsTmpDir } from "openclaw/plugin-sdk";
// NOTE: This extension is intended to be bundled with QingClaws.
// When running from source (tests/dev), QingClaws internals live under src/.
...
export function createLlmTaskTool(api: QingClawsPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via qingclaws.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredQingClawsTmpDir(), "qingclaws-llm-task-"),
        );
