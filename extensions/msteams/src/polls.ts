...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["qingclawsPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["qingclaws", "pollId"]) ??
    readNestedString(value, ["qingclaws", "poll", "id"]) ??
    readNestedString(value, ["data", "qingclawsPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "qingclaws", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          qingclawsPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "qingclaws poll vote",
          displayText: "Vote recorded",
          value: { qingclawsPollId: pollId, pollId },
        },
      },
    ],
  };
