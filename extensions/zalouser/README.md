...
QingClaws extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
qingclaws channels login --channel zalouser
...
```bash
qingclaws message send --channel zalouser --target <threadId> --message "Hello from QingClaws!"
```
...
```bash
qingclaws channels login --channel zalouser              # Login via QR
qingclaws channels login --channel zalouser --account work
qingclaws channels status --probe
qingclaws channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
qingclaws directory self --channel zalouser
qingclaws directory peers list --channel zalouser --query "name"
qingclaws directory groups list --channel zalouser --query "work"
qingclaws directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
qingclaws message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
qingclaws message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `qingclaws channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
qingclaws channels login --channel zalouser --account work
qingclaws message send --channel zalouser --account work --target <id> --message "Hello"
