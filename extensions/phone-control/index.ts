...
import type { QingClawsPluginApi, QingClawsPluginService } from "openclaw/plugin-sdk";
...
function normalizeDenyList(cfg: QingClawsPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: QingClawsPluginApi["config"]): string[] {
...
export default function register(api: QingClawsPluginApi) {
  ...
  const timerService: QingClawsPluginService = {
