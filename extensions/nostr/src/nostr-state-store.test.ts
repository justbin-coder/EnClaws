...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.QINGCLAWS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "qingclaws-nostr-"));
  process.env.QINGCLAWS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.QINGCLAWS_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".qingclaws");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.QINGCLAWS_STATE_DIR = previous;
    }
