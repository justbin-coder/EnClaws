...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  return [...document.querySelectorAll<HTMLElement>(".oc-diff-card")].flatMap((card) => {
    const host = card.querySelector<HTMLElement>("[data-qingclaws-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-qingclaws-diff-payload]");
    if (!host || !payloadNode) {
      return [];
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.qingclawsDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.qingclawsDiffsError = "true";
