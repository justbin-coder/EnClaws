...
function resolveQingClawsRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.QINGCLAWS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set QINGCLAWS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveQingClawsRoot(), "dist", "extensionAPI.js");
