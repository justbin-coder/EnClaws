# Google Gemini CLI Auth (QingClaws plugin)
...
```bash
qingclaws plugins enable google-gemini-cli-auth
```
...
```bash
qingclaws models auth login --provider google-gemini-cli --set-default
```
...
- `QINGCLAWS_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `QINGCLAWS_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
