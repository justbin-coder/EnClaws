...
metadata: { "qingclaws": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == QingClaws `sessions_spawn`
- **File I/O** == QingClaws `read`/`write`
- **Remote fetch** == QingClaws `web_fetch` (or `exec` with curl when POST is required)
