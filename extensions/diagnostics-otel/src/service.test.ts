...
import type { QingClawsPluginServiceContext } from "openclaw/plugin-sdk";
import { emitDiagnosticEvent } from "openclaw/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/qingclaws-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): QingClawsPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("qingclaws.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("qingclaws.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("qingclaws.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("qingclaws.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("qingclaws.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("qingclaws.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("qingclaws.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("qingclaws.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("qingclaws.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("qingclaws.webhook.processed");
    expect(spanNames).toContain("qingclaws.message.processed");
    expect(spanNames).toContain("qingclaws.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["qingclaws.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("qingclaws.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "qingclaws.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["qingclaws.reason"]).toBe("string");
    expect(String(attrs?.["qingclaws.reason"])).not.toContain(
