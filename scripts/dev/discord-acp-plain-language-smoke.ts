...
type DriverMode = "token" | "webhook" | "qingclaws";
...
function resolveStateDir(): string {
  const override = process.env.QINGCLAWS_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.QINGCLAWS_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".qingclaws");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver qingclaws] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) QingClaws spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|qingclaws> Driver transport mode (default: token)\n" +
    ...
    "  --qingclaws-bin <path>        QingClaws CLI binary for driver=qingclaws (default: qingclaws)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  QINGCLAWS_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  QINGCLAWS_DISCORD_SMOKE_DRIVER\n" +
    "  QINGCLAWS_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  QINGCLAWS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  QINGCLAWS_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  QINGCLAWS_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  QINGCLAWS_DISCORD_SMOKE_AGENT\n" +
    "  QINGCLAWS_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  QINGCLAWS_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  QINGCLAWS_DISCORD_SMOKE_POLL_MS\n" +
    "  QINGCLAWS_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  QINGCLAWS_DISCORD_SMOKE_QINGCLAWS_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_DRIVER ||
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "qingclaws"
        ? "qingclaws"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.QINGCLAWS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.QINGCLAWS_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.QINGCLAWS_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.QINGCLAWS_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.QINGCLAWS_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const qingclawsBin =
    resolveArg("--qingclaws-bin") || process.env.QINGCLAWS_DISCORD_SMOKE_QINGCLAWS_BIN || "qingclaws";
...
async function qingclawsCliJson<T>(params: { qingclawsBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.qingclawsBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`qingclaws ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await qingclawsCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    qingclawsBin: params.qingclawsBin,
    args: [
      "message",
      "read",
      "--channel",
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `qingclaws-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await qingclawsCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        qingclawsBin: args.qingclawsBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("qingclaws message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from QingClaws.`,
        diagnostics: {
          bindingCandidates: [
            {
