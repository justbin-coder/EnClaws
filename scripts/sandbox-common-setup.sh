...
BASE_IMAGE="${BASE_IMAGE:-qingclaws-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-qingclaws-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=qingclaws.sandbox=1)
NOTE
