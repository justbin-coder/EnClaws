...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/qingclaws/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "QingClaws Re-Auth" -c "After getting key, run: ssh $SERVER '~/qingclaws/scripts/mobile-reauth.sh'" --id qingclaws-auth
        ;;
    *)
