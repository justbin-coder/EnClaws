...
# Checks both Claude Code and QingClaws auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
QINGCLAWS_AUTH="$HOME/.qingclaws/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    qingclaws models status --json 2>/dev/null || true
}
...
check_qingclaws_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$QINGCLAWS_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    qingclaws_status=$(check_qingclaws_auth 2>/dev/null || true)

    claude_expires=0
    qingclaws_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        qingclaws_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        qingclaws_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$QINGCLAWS_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$qingclaws_status" \
        --arg be "$qingclaws_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            qingclaws: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$qingclaws_status" == EXPIRED* ]] || [[ "$qingclaws_status" == MISSING* ]]; then
        echo "QINGCLAWS_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$qingclaws_status" == EXPIRING* ]]; then
        echo "QINGCLAWS_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "QingClaws Auth (~/.qingclaws/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$QINGCLAWS_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$QINGCLAWS_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'qingclaws doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active qingclaws >/dev/null 2>&1; then
    echo -e "QingClaws service: ${GREEN}running${NC}"
else
    echo -e "QingClaws service: ${RED}NOT running${NC}"
fi
