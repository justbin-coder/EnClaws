#!/usr/bin/env bash
# ClawDock - Docker helpers for QingClaws
# Inspired by Simon Willison's "Running QingClaws in Docker"
# https://til.simonwillison.net/llms/qingclaws-docker
#
# Installation:
...
# Common paths to check for QingClaws
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/qingclaws"
  "${HOME}/workspace/qingclaws"
  "${HOME}/projects/qingclaws"
  "${HOME}/dev/qingclaws"
  "${HOME}/code/qingclaws"
  "${HOME}/src/qingclaws"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found QingClaws at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/qingclaws"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ QingClaws not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/openclaw/openclaw.git ~/openclaw"
    echo "  cd ~/qingclaws && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^QINGCLAWS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d qingclaws-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart qingclaws-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f qingclaws-gateway
}
...
clawdock-workspace() {
  cd ~/.qingclaws/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec qingclaws-gateway \
    bash -c 'echo "alias qingclaws=\"./qingclaws.mjs\"" > /tmp/.bashrc_qingclaws && bash --rcfile /tmp/.bashrc_qingclaws'
}
...
clawdock-cli() {
  _clawdock_compose run --rm qingclaws-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build qingclaws-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "QINGCLAWS_GATEWAY_TOKEN=$token" qingclaws-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" qingclaws-gateway \
    bash -c './qingclaws.mjs config set gateway.remote.token "$TOKEN" && ./qingclaws.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec qingclaws-gateway \
    bash -c "./qingclaws.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart qingclaws-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm qingclaws-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec qingclaws-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'qingclaws config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for QingClaws${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (qingclaws alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to qingclaws project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.qingclaws)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'qingclaws channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'qingclaws status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.qingclaws.ai${_CLR_RESET}"
  echo ""
}
