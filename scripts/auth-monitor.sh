...
#
# Suggested cron: */30 * * * * /home/admin/qingclaws/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send QingClaws notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., qingclaws-alerts)
...
STATE_FILE="$HOME/.qingclaws/auth-monitor-state"
...
send_notification() {
    ...
    # Send via QingClaws if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use qingclaws
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via QingClaws to $NOTIFY_PHONE..."
            qingclaws send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! QingClaws is down. Run: ssh l36 '~/qingclaws/scripts/mobile-reauth.sh'" "urgent"
    exit 1
