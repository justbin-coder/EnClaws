...
IMAGE_NAME="qingclaws-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="qingclaws-net-e2e-$$"
GW_NAME="qingclaws-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "QINGCLAWS_GATEWAY_TOKEN=$TOKEN" \
  -e "QINGCLAWS_SKIP_CHANNELS=1" \
  -e "QINGCLAWS_SKIP_GMAIL_WATCHER=1" \
  -e "QINGCLAWS_SKIP_CRON=1" \
  -e "QINGCLAWS_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
