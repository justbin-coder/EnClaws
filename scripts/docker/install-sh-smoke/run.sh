...
INSTALL_URL="${QINGCLAWS_INSTALL_URL:-https://qingclaws.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${QINGCLAWS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${QINGCLAWS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="qingclaws"
PACKAGE_NAME="${QINGCLAWS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (QINGCLAWS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${QINGCLAWS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${QINGCLAWS_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
