#!/bin/bash
# Setup QingClaws Auth Management System
...
echo "=== QingClaws Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'qingclaws-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  QingClaws message: Send warning via QingClaws itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/qingclaws-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/qingclaws-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/qingclaws-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now qingclaws-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status qingclaws-auth-monitor.timer"
echo ""
echo "Quick commands:"
