 ...
 * `qingclaws update`. Use it to verify the completion installation flow
...
\`qingclaws update\`, \`qingclaws doctor\`, and \`qingclaws onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# QingClaws Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`qingclaws completion --install\` later to enable.`));
    return;
  }
