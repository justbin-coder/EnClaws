...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "QingClaws")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "QingClaws")
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/qingclaws-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/qingclaws-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/qingclaws-dmg-limits.txt
