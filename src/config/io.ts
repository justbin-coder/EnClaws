...
import { compareQingClawsVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  qingclaws config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  qingclaws config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function warnIfConfigFromFuture(cfg: OpenClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  const cmp = compareQingClawsVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer QingClaws (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFile(cfg: OpenClawConfig, options: ConfigWriteOptions = {}) {
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.QINGCLAWS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.QINGCLAWS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.QINGCLAWS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.QINGCLAWS_WATCH_SESSION === "string" &&
        deps.env.QINGCLAWS_WATCH_SESSION.trim().length > 0
          ? deps.env.QINGCLAWS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.QINGCLAWS_WATCH_COMMAND === "string" &&
        deps.env.QINGCLAWS_WATCH_COMMAND.trim().length > 0
          ? deps.env.QINGCLAWS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `QINGCLAWS_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.QINGCLAWS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.QINGCLAWS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
