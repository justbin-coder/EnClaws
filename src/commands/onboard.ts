...
export async function onboardCommand(opts: OnboardOptions, runtime: RuntimeEnv = defaultRuntime) {
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://docs.qingclaws.ai/security",
        `Re-run with: ${formatCliCommand("qingclaws onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "检测到 Windows 系统 — QingClaws 在 WSL2 上的运行效果最佳！",
        "原生的 Windows 支持可能还会遇到一些问题。",
        "推荐的快速安装方式：使用 wsl --install (只需一条命令并重启一次)",
        "参考指南：https://docs.qingclaws.ai/windows",
      ].join("\n"),
    );
  }
