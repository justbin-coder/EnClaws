...
const SEARCH_TOOL = "https://docs.qingclaws.ai/mcp.SearchQingClaws";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.qingclaws.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('qingclaws docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.qingclaws.ai/");
      runtime.log(`Search: ${formatCliCommand('qingclaws docs "your query"')}`);
    }
    return;
  }
