...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "qingclaws-doctor-"));
  tempRoot = root;
  return root;
}
...
async function detectAndRunMigrations(params: {
  ...
  const detected = await detectLegacyStateMigrations({
    cfg: params.cfg,
    env: { QINGCLAWS_STATE_DIR: params.root } as NodeJS.ProcessEnv,
  });
...
function getStateDirMigrationPaths(root: string) {
  return {
    targetDir: path.join(root, ".qingclaws"),
...
describe("doctor legacy state migrations", () => {
  it("migrates legacy sessions into agents/<id>/sessions", async () => {
    ...
    const detected = await detectLegacyStateMigrations({
      cfg,
      env: { QINGCLAWS_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
