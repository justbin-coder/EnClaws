...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "QINGCLAWS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("QINGCLAWS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv QINGCLAWS_GATEWAY_TOKEN");
    expect(message).not.toContain("QINGCLAWS_GATEWAY_PASSWORD");
  });
