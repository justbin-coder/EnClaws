...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveQingClawsAgentDir: vi.fn().mockReturnValue("/tmp/qingclaws-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/qingclaws-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveQingClawsAgentDir: mocks.resolveQingClawsAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/qingclaws-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveQingClawsAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/qingclaws-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/qingclaws-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/qingclaws-agent-custom");
