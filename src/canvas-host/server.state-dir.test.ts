...
describe("canvas host state dir defaults", () => {
  it("uses QINGCLAWS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("qingclaws-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("QingClaws Canvas");
      } finally {
        await handler.close();
      }
    });
