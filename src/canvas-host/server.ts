...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.qingclawsCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.qingclawsCanvasA2UIAction &&
        typeof window.qingclawsCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.qingclawsSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("qingclaws:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android QingClaws node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.qingclawsSendUserAction === "function"
        ? window.qingclawsSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.QINGCLAWS_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.QINGCLAWS_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>QingClaws Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
