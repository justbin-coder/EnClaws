...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.qingclaws.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + QingClaws hooks")
    ...
    .option("--hook-url <url>", "QingClaws hook URL")
    .option("--hook-token <token>", "QingClaws hook token")
