...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "qingclaws", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "qingclaws", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "qingclaws", "status"]);
  });
