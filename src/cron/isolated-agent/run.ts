...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/qingclaws/qingclaws/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  agentId?: string;
  lane?: string;
  /** Multi-tenant context — when set, workspace and agent dirs resolve under the tenant tree. */
  tenantId?: string;
  userId?: string;
  ...
  const isFastTestEnv = process.env.QINGCLAWS_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.qingclaws/agents/<agentId>/agent/).
  ...
  // Multi-tenant delivery resolution.
  //
  // In multi-tenant mode, resolveDeliveryTarget reads the single-tenant
  // session store path which doesn't contain tenant user sessions.  This
  // causes it to pick up stale data (e.g. an old feishu session from a
  // different user) and attempt delivery to the wrong channel.
  //
  // Fix: read the *tenant-scoped* session store first.  For webchat users,
  // bypass resolveDeliveryTarget entirely (it would ignore "webchat" as a
  // non-deliverable channel and fall back to stale data).  For feishu users,
  // inject the correct target so resolveDeliveryTarget uses it directly.
  let deliveryTo = deliveryPlan.to;
  let deliveryChannel = deliveryPlan.channel ?? "last";
  let deliveryRequested = deliveryPlan.requested;
  let tenantWebchatDelivery = false;
  let tenantAnnounceSessionKey: string | undefined;
  let tenantLastAccountId: string | undefined;
  ...
  if (params.tenantId && params.userId) {
    ...
    // (e.g. "t:{tenantId}:agent:2222:chat") and may use a non-default
    // session name.  Instead of guessing the exact key, find the most
    // recently updated session entry that has a lastChannel set.
    ...
    for (const [key, entry] of Object.entries(sStore)) {
      ...
      if (updatedAt >= latestUpdatedAt) {
        ...
        lastChannel = entry.lastChannel.trim().toLowerCase();
        lastTo = entry.lastTo?.trim();
        tenantLastAccountId = entry.lastAccountId?.trim() || undefined;
        tenantAnnounceSessionKey = key;
      }
      // Extract feishu open_id (ou_xxx) from any session entry's lastTo
      // for @mention in group chat delivery.
      if (!tenantFeishuOpenId) {
        const entryTo = entry.lastTo?.trim() ?? "";
        const ouMatch = entryTo.match(/(?:^|user:)(ou_[a-f0-9]+)/i);
        if (ouMatch) {
          tenantFeishuOpenId = ouMatch[1];
        }
      }
    }

    // Only fill in deliveryTo/channel from the session store when the job
    // does not already have an explicit delivery target.
    if (!deliveryTo) {
      if (lastChannel === "webchat") {
        ...
        // No prior session or unknown channel — check if userId itself is a
        // Feishu openid (supports future migration where directory = openid).
        const uid = params.userId.trim().toLowerCase();
        if (uid.startsWith("ou_") || uid.startsWith("on_") || uid.startsWith("oc_")) {
          deliveryTo = params.userId;
  ...
  // That function treats "webchat" as non-deliverable and falls back to the
  // single-tenant session store, picking up stale external channel data.
  // Instead, construct an ok:false/implicit result so the dispatch logic
  // falls through to announceToMainSession (pushes output via WebSocket).
  const resolvedDelivery: DeliveryTargetResolution = tenantWebchatDelivery
    ? {
        ok: false,
    ...
    : await resolveDeliveryTarget(cfgWithAgentDefaults, agentId, {
        ...
        to: deliveryTo,
        accountId: deliveryPlan.accountId ?? tenantLastAccountId,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/qingclaws/qingclaws/issues/29774
  ...
  let deliveryPayloads =
    deliveryPayload !== undefined
      ...
      : synthesizedText
        ...
        : [];
  // For Feishu group sessions, prepend @mention of the original sender to delivery payloads.
  // The session key encodes the sender as ":feishu:group:<chatId>:sender:<openId>".
  const feishuGroupSenderMatch = params.job.sessionKey?.match(
    /:feishu:group:[^:]+:sender:(ou_[^:]+)/,
  );
  const feishuSenderOpenId = feishuGroupSenderMatch?.[1] ?? tenantFeishuOpenId;
  const deliveryIsFeishu =
    resolvedDelivery.channel === "feishu" ||
    resolvedDelivery.channel === "lark" ||
    deliveryChannel === "feishu" ||
    deliveryChannel === "lark";
  if (feishuSenderOpenId && deliveryIsFeishu) {
