...
vi.mock("./chrome.js", () => ({
  resolveQingClawsUserDataDir: vi.fn(() => "/tmp/qingclaws-test/qingclaws/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "qingclaws-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveQingClawsUserDataDir).mockReturnValue(userDataDir);
