...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/qingclaws" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          qingclaws: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchQingClawsChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.QINGCLAWS_GATEWAY_PORT;
  process.env.QINGCLAWS_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.QINGCLAWS_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.QINGCLAWS_GATEWAY_PASSWORD;
