...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredQingClawsTmpDir: vi.fn(() => "/tmp/qingclaws"),
}));
vi.mock("../infra/tmp-qingclaws-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredQingClawsTmpDir.mockReturnValue("/tmp/qingclaws");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "qingclaws-browser-download-test-"));
    try {
  ...
  it("waits for the next download and atomically finalizes explicit output paths", async () => {
    await withTempDir(async (tempDir) => {
      ...
      expect(path.basename(String(savedPath))).toContain(".qingclaws-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredQingClawsTmpDir.mockReturnValue("/tmp/qingclaws-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "qingclaws-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "qingclaws-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredQingClawsTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "qingclaws-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "qingclaws-preferred", "downloads")}${path.sep}`),
    );
  });
