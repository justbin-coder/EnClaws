...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".qingclaws-profile-decorated");
}
 ...
 */
export function decorateQingClawsProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_QINGCLAWS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_QINGCLAWS_BROWSER_COLOR).toUpperCase();
