...
const { createQingClawsTools } = await import("../agents/qingclaws-tools.js");
...
type SessionSendTool = ReturnType<typeof createQingClawsTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createQingClawsTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["QINGCLAWS_GATEWAY_PORT", "QINGCLAWS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.QINGCLAWS_GATEWAY_PORT = String(gatewayPort);
  process.env.QINGCLAWS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("QINGCLAWS_CONFIG_PATH missing in gateway test environment");
      }
