...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("qingclaws doctor --fix");
  });
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["QINGCLAWS_GATEWAY_TOKEN", "QINGCLAWS_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.QINGCLAWS_GATEWAY_TOKEN = "env-token";
    process.env.QINGCLAWS_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "QINGCLAWS_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      label: "token",
      authKey: "token",
      envKey: "QINGCLAWS_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["QINGCLAWS_GATEWAY_PASSWORD", "QINGCLAWS_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.QINGCLAWS_GATEWAY_PASSWORD = envPassword;
    }
