...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.QINGCLAWS_SKIP_CHANNELS;
    prevSkipGmail = process.env.QINGCLAWS_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.QINGCLAWS_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.QINGCLAWS_SKIP_CHANNELS = "0";
    delete process.env.QINGCLAWS_SKIP_GMAIL_WATCHER;
    delete process.env.QINGCLAWS_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.QINGCLAWS_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.QINGCLAWS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.QINGCLAWS_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.QINGCLAWS_SKIP_PROVIDERS;
    } else {
      process.env.QINGCLAWS_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeEnvRefConfig() {
    ...
    if (!configPath) {
      throw new Error("QINGCLAWS_CONFIG_PATH is not set");
    }
    await fs.writeFile(
      configPath,
      `${JSON.stringify(
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.QINGCLAWS_STATE_DIR;
    if (!stateDir) {
      throw new Error("QINGCLAWS_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_QINGCLAWS_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_QINGCLAWS_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_QINGCLAWS_AUTH_REF" is missing or empty.',
      );
    } finally {
