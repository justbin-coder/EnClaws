...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.QINGCLAWS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*qingclaws-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "qingclaws";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "qingclaws");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "qingclaws-gw");
  ...
  records.push(`_qingclaws-gw._tcp IN PTR ${instanceLabel}._qingclaws-gw._tcp`);
  records.push(`${instanceLabel}._qingclaws-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._qingclaws-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; qingclaws-content-hash: ${contentHash}\n${contentBody}`;
}
