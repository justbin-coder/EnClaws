...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.QINGCLAWS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.QINGCLAWS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set QINGCLAWS_APNS_TEAM_ID and QINGCLAWS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.QINGCLAWS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.QINGCLAWS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.QINGCLAWS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set QINGCLAWS_APNS_PRIVATE_KEY_P8 or QINGCLAWS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading QINGCLAWS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createQingClawsPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    qingclaws: createQingClawsPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
