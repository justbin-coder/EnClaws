...
vi.mock("../../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: () => "/tmp/qingclaws-test-workspace",
}));
...
vi.mock("../../plugins/loader.js", () => ({
  loadQingClawsPlugins: mocks.loadQingClawsPlugins,
}));
...
describe("resolveOutboundTarget channel resolution", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadQingClawsPlugins.mockReset();
  });
  ...
  it("recovers telegram plugin resolution so announce delivery does not fail with Unsupported channel: telegram", () => {
    ...
    expect(mocks.loadQingClawsPlugins).toHaveBeenCalledTimes(1);
  });
  ...
  it("retries bootstrap on subsequent resolve when the first bootstrap attempt fails", () => {
    ...
    expect(mocks.loadQingClawsPlugins).toHaveBeenCalledTimes(2);
  });
});
