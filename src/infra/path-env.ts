...
function candidateBinDirs(opts: EnsureQingClawsPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `qingclaws` lives next to the executable (process.execPath).
  try {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.QINGCLAWS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "qingclaws"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `qingclaws` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureQingClawsCliOnPath(opts: EnsureQingClawsPathOpts = {}) {
  if (isTruthyEnvValue(process.env.QINGCLAWS_PATH_BOOTSTRAPPED)) {
    return;
  }
