...
vi.mock("./qingclaws-root.js", () => ({
  resolveQingClawsPackageRoot: vi.fn(async () => null),
  resolveQingClawsPackageRootSync: vi.fn(() => null),
}));
...
let qingclawsRoot: typeof import("./qingclaws-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    qingclawsRoot = await import("./qingclaws-root.js");
  });
  ...
  it("uses resolveQingClawsPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/qingclaws");
    (
      qingclawsRoot.resolveQingClawsPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/qingclaws"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "qingclaws" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "qingclaws.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-qingclaws");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/qingclaws-bundle");
    (
      qingclawsRoot.resolveQingClawsPackageRootSync as unknown as ReturnType<typeof vi.fn>
