...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses QINGCLAWS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      QINGCLAWS_HOME: "/srv/qingclaws-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/qingclaws-home"), ".qingclaws", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".qingclaws", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("qingclaws-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "qingclaws-workspace-hardlink-"));
    try {
