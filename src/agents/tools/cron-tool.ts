...
type CronToolOptions = {
  ...
  /** Tenant ID for multi-tenant scoped cron operations. */
  tenantId?: string;
  /** Tenant user ID for multi-tenant scoped cron operations. */
  tenantUserId?: string;
};
...
/**
 * Lightweight format check: does `peerId` look like a plausible delivery target
 * for `channel`?  This is intentionally conservative – it only rejects IDs that
 * are clearly *not* valid for the channel.  Unknown channels pass through.
 */
function isPlausiblePeerIdForChannel(channel: string, peerId: string): boolean {
  const id = peerId.trim().toLowerCase();
  if (!id) {
    return false;
  }
  switch (channel) {
    // Feishu / Lark: valid targets start with ou_ (open_id), on_ (union_id),
    // or oc_ (chat_id).
    case "feishu":
    case "lark":
      return /^(ou_|on_|oc_)/.test(id);
    ...
    // Slack: user IDs start with U/W, channel IDs with C/D/G.
    case "slack":
      return /^[uwcdg]/i.test(id);

    default:
      // Unknown channel – allow through; don't block channels we don't know
      // about (including plugin channels).
      return true;
  }
}
...
function resolveFeishuOpenIdFromTenantStore(
  ...
  try {
    ...
    for (const entry of Object.values(store)) {
      const to = (entry as { lastTo?: string } | undefined)?.lastTo?.trim() ?? "";
      const match = to.match(/(?:^|user:)(ou_[a-f0-9]+)/i);
      if (match) {
        return match[1];
      }
    }
  } catch {
    // Best-effort: if session store is unavailable, skip enrichment.
  }
  return undefined;
}
...
function inferDeliveryFromSessionKey(agentSessionKey?: string): CronDelivery | null {
  ...
  // Validate that the peer ID looks like a valid identifier for the inferred
  // channel.  In multi-tenant setups the session key may carry the channel type
  // from the tenant_channels DB record (e.g. "feishu") while the actual peer ID
  // is an internal web-console user name (e.g. "heartbeat") that is not a valid
  // delivery target for that channel.  Returning null here causes the caller to
  // fall back to the session-injection path, which correctly reaches web-UI
...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  return {
    ...
    name: "cron",
    ownerOnly: true,
    tenantScoped: !!(opts?.tenantId && opts?.tenantUserId),
...
Use jobId as the canonical identifier; id is accepted for compatibility. Use contextMessages (0-10) to add previous messages as context to the job text.`,
    ...
    execute: async (_toolCallId, args) => {
      ...
      // Inject tenant context into gateway call params so the server-side handler
      ...
      switch (action) {
        case "status":
          return jsonResult(await callGateway("cron.status", gatewayOpts, { ...tenantParams }));
        case "list":
          return jsonResult(
        ...
        case "add": {
          ...
          // See: https://github.com/qingclaws/qingclaws/issues/11310
          if (
            !params.job ||
          ...
          if (job && typeof job === "object") {
            ...
            if (!("sessionKey" in job) && resolvedSessionKey) {
              // For Feishu/Lark group sessions in multi-tenant mode, append
              // ":sender:<openId>" so the cron delivery can @mention the user
              // who created the job.  The open_id is resolved from the tenant
              // session store (the main session records lastTo as "user:ou_xxx").
              let enrichedSessionKey = resolvedSessionKey;
              if (
                opts?.tenantId &&
                opts?.tenantUserId &&
        ...
        case "remove": {
          ...
          return jsonResult(await callGateway("cron.remove", gatewayOpts, { id, ...tenantParams }));
        }
        case "run": {
          const id = readStringParam(params, "jobId") ?? readStringParam(params, "id");
          if (!id) {
        ...
        case "runs": {
          ...
          return jsonResult(await callGateway("cron.runs", gatewayOpts, { id, ...tenantParams }));
        }
        case "wake": {
