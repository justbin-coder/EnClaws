...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    qingclaws: process.env.QINGCLAWS_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.qingclaws === undefined) {
      delete process.env.QINGCLAWS_GATEWAY_TOKEN;
    } else {
      process.env.QINGCLAWS_GATEWAY_TOKEN = envSnapshot.qingclaws;
    }
  ...
  it("uses QINGCLAWS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.QINGCLAWS_GATEWAY_TOKEN = "local-env-token";
