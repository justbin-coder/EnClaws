...
type CoreToolDefinition = {
  ...
  includeInQingClawsGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const qingclawsTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInQingClawsGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:qingclaws": qingclawsTools,
