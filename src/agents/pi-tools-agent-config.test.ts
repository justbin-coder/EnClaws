...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "qingclaws-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OpenClawConfig) {
    return createQingClawsCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const cfg: OpenClawConfig = {
      ...
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/qingclaws-restricted",
            tools: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createQingClawsCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createQingClawsCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createQingClawsCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createQingClawsCodingTools({
      config: cfg,
    ...
    const bobTools = createQingClawsCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createQingClawsCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createQingClawsCodingTools({
      config: cfg,
