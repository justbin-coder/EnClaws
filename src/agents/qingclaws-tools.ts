...
export function createQingClawsTools(options?: {
  ...
  // Tenant memory tool (multi-tenant mode only)
  const tenantMemoryTool = createTenantMemoryTool({ tenantId: options?.tenantId });
  if (tenantMemoryTool) {
    tools.push(tenantMemoryTool);
  }

  // User memory tool (multi-tenant mode only, requires both tenantId and userId)
  const userMemoryTool = createUserMemoryTool({
    tenantId: options?.tenantId,
    userId: options?.tenantUserId,
  });
  if (userMemoryTool) {
    tools.push(userMemoryTool);
  }
