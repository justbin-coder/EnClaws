...
describe("resolveQingClawsAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "qingclaws-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolveQingClawsAgentDir();
  ...
  it("honors QINGCLAWS_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          QINGCLAWS_STATE_DIR: undefined,
          QINGCLAWS_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveQingClawsAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when QINGCLAWS_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers QINGCLAWS_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          QINGCLAWS_STATE_DIR: undefined,
          QINGCLAWS_AGENT_DIR: primaryOverride,
