...
describe("createQingClawsCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "qingclaws-read-"));
    try {
      ...
      const contents = "Hello from qingclaws read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createQingClawsCodingTools({ sandbox });
