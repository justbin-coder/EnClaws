...
const GOOGLE_LIVE_SINGLE_KEY = "QINGCLAWS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "QINGCLAWS_LIVE_ANTHROPIC_KEY",
    listVar: "QINGCLAWS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "QINGCLAWS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `QINGCLAWS_LIVE_${base}_KEY`;
