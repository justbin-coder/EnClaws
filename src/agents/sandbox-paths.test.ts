...
async function withOutsideHardlinkInQingClawsTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside qingclaws tmp root",
      media: path.join(os.tmpdir(), "outside-qingclaws", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked QingClaws tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked QingClaws tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInQingClawsTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked QingClaws tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInQingClawsTmp(
