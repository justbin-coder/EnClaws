...
import {
  ...
  normalizeStringList,
  parseQingClawsManifestInstallBase,
  parseFrontmatterBool,
  resolveQingClawsManifestBlock,
  resolveQingClawsManifestInstall,
  resolveQingClawsManifestOs,
  resolveQingClawsManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  QingClawsSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseQingClawsManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveQingClawsMetadata(
  frontmatter: ParsedSkillFrontmatter,
): QingClawsSkillMetadata | undefined {
  const metadataObj = resolveQingClawsManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveQingClawsManifestRequires(metadataObj);
  const install = resolveQingClawsManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveQingClawsManifestOs(metadataObj);
  return {
