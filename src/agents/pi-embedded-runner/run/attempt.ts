...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      // LLM interaction trace recorder (multi-tenant only)
      const interactionTraceRecorder = params.tenantId
        ? createInteractionTraceRecorder(
            {
              tenantId: params.tenantId,
              tenantUserId: params.tenantUserId,
              sessionKey: params.sessionKey,
      ...
      // Interaction trace wrapper (outermost - sees final request payloads)
      if (interactionTraceRecorder) {
        activeSession.agent.streamFn = interactionTraceRecorder.wrapStreamFn(
          activeSession.agent.streamFn,
        );
      }
      ...
      try {
        ...
        try {
          ...
          // Capture system prompt for interaction trace before calling LLM
          if (interactionTraceRecorder && systemPromptText) {
            interactionTraceRecorder.setSystemPrompt(systemPromptText);
          }
        ...
        // Record interaction trace (fire-and-forget)
        if (interactionTraceRecorder) {
          interactionTraceRecorder.recordRound(activeSession.messages, promptError ?? undefined);
        }
