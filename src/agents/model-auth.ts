...
export async function resolveApiKeyForProvider(params: {
  ...
  // Explicit apiKey in models.providers takes priority over auth profiles.
  // This ensures tenant-configured provider keys are used directly.
  const customKey = getCustomProviderApiKey(cfg, provider);
  if (customKey) {
    return { apiKey: customKey, source: "models.providers", mode: "api-key" };
  }
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("qingclaws agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
