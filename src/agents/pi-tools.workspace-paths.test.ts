...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("qingclaws-ws-", async (workspaceDir) => {
      await withTempDir("qingclaws-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createQingClawsCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("qingclaws-ws-", async (workspaceDir) => {
      await withTempDir("qingclaws-override-", async (overrideDir) => {
        const tools = createQingClawsCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("qingclaws-ws-", async (workspaceDir) => {
      ...
      const tools = createQingClawsCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-qingclaws.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("qingclaws-sandbox-", async (sandboxDir) => {
      await withTempDir("qingclaws-workspace-", async (workspaceDir) => {
        ...
        const tools = createQingClawsCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
