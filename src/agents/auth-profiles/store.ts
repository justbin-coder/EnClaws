 ...
 * A common mistake — caused by the similarity with the `qingclaws.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (qingclaws.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.QINGCLAWS_AUTH_STORE_READONLY === "1";
