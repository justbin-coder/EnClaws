...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["QINGCLAWS_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "qingclaws-subagent-"));
  ...
  it("uses isolated temp state when QINGCLAWS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "qingclaws-test-state"));
  });
});
