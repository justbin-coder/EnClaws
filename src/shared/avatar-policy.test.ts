...
describe("avatar policy", () => {
  it("accepts workspace-relative avatar paths and rejects URI schemes", () => {
    expect(isWorkspaceRelativeAvatarPath("avatars/qingclaws.png")).toBe(true);
    expect(isWorkspaceRelativeAvatarPath("C:\\\\avatars\\.qingclaws.png")).toBe(true);
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/qingclaws.svg")).toBe(true);
    expect(looksLikeAvatarPath("qingclaws.webp")).toBe(true);
