...
const testLogPath = path.join(os.tmpdir(), "qingclaws-test-env-log-level.log");
const defaultMaxFileBytes = 500 * 1024 * 1024;

describe("QINGCLAWS_LOG_LEVEL", () => {
  let originalEnv: string | undefined;

  beforeEach(() => {
    originalEnv = process.env.QINGCLAWS_LOG_LEVEL;
    delete process.env.QINGCLAWS_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.QINGCLAWS_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.QINGCLAWS_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.QINGCLAWS_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("QINGCLAWS_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid QINGCLAWS_LOG_LEVEL="nope"');
  });
});
