...
export const DEFAULT_LOG_DIR = resolvePreferredQingClawsTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "qingclaws.log"); // legacy single-file path

const LOG_PREFIX = "qingclaws";
...
function resolveSettings(): ResolvedSettings {
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.QINGCLAWS_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[qingclaws] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
