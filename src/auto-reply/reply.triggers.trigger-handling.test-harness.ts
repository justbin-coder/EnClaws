...
type TempHomeEnvSnapshot = {
  ...
  qingclawsHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    qingclawsHome: process.env.QINGCLAWS_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("QINGCLAWS_HOME", snapshot.qingclawsHome);
  restoreKey("QINGCLAWS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.QINGCLAWS_STATE_DIR = join(home, ".qingclaws");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "qingclaws-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".qingclaws", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
