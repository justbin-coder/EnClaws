...
import { scheduleGatewaySigusr1Restart, triggerQingClawsRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting QingClaws in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerQingClawsRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting QingClaws via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
