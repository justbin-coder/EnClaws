...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".qingclaws");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.QINGCLAWS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$QINGCLAWS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via QINGCLAWS_STATE_DIR.
